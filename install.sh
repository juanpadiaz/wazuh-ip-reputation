...
# Versión: 2.0.2 (con parche integrado)
# =============================================================================
...
EOF
    ...
    echo "   ✅ Extracción mejorada de IPs de Wazuh"
    echo
...
('database_version', '2.0.2', 'Versión del esquema de base de datos'),
...
create_main_application() {
    log_step "Creando aplicación principal con método mejorado de extracción de IPs..."
    
    cat > "$INSTALL_DIR/wazuh_ip_reputation.py" << 'APPEOF'
#!/usr/bin/env python3
"""
Wazuh IP Reputation Checker v2.0.2
Analiza IPs de logs de Wazuh y verifica su reputación
Versión con extracción mejorada de IPs
"""
...
class WazuhIPReputationChecker:
    def __init__(self, config_file: str = '/etc/wazuh-ip-reputation/config.ini'):
        ...
        self.logger.info("Wazuh IP Reputation Checker v2.0.2 iniciado")
    ...
    def extract_ips_from_wazuh(self) -> List[Dict]:
        ...
        try:
            ...
            # Primero obtener lista de agentes activos
            ...
            # Para cada agente, obtener eventos de seguridad recientes
            for agent in agents:
                agent_id = agent.get('id', '000')
                agent_name = agent.get('name', 'Unknown')
                
                # Saltar el manager (id 000)
                if agent_id == '000':
                    continue
                
                try:
                    # Obtener eventos de vulnerabilidad del agente
                    ...
                    if vuln_response.status_code == 200:
                        ...
                        # Buscar IPs en las vulnerabilidades
                    ...
                    # Obtener eventos de integridad del agente
                    ...
                    if syscheck_response.status_code == 200:
                        ...
                        # Buscar IPs en eventos de integridad
                    ...
                    self.logger.debug(f"Error procesando agente {agent_id}: {e}")
            
            # Si no encontramos IPs en los eventos, buscar en los logs del sistema
            if len(extracted_ips) == 0:
                self.logger.info("No se encontraron IPs en eventos, buscando en logs del sistema...")
                
                # Obtener el resumen de logs del manager
                try:
                    manager_logs_url = f"{base_url}/manager/logs/summary"
                    ...
                    if logs_response.status_code == 200:
                        ...
                        # Buscar IPs en los logs
                        logs_str = str(logs_data)
                        found_ips = ip_pattern.findall(logs_str)
                        
                        for ip in found_ips:
                            if self._is_valid_public_ip(ip):
                                ip_data = {
                                    'ip': ip,
                                    'alert_id': 'manager-log',
                                    'agent_id': '000',
                                    'agent_name': 'wazuh-manager',
                                    'rule_id': 'log',
                                    'rule_description': 'IP found in manager logs',
                                    'timestamp': datetime.now().isoformat()
                                }
                                extracted_ips.append(ip_data)
                                
                except Exception as e:
                    self.logger.debug(f"Error obteniendo logs del manager: {e}")
            
            # Como última opción, generar IPs de prueba si está en modo de prueba
            ...
            self.logger.info(f"Extraídas {len(final_ips)} IPs únicas de Wazuh")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Wazuh IP Reputation Checker v2.0.2')
...
APPEOF
    ...
    log_success "Aplicación principal creada con método de extracción mejorado"
}
...
show_summary() {
    ...
    echo -e "${GREEN}Versión con extracción mejorada de IPs instalada${NC}"
}
